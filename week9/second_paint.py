...
def drawRectangle(screen, start, end, width, color):
    ...
    if x2 > x1 and y2 > y1:
        pygame.draw.rect(screen, pygame.Color(color), (x1, y1, widthr, height),
                         width)  # Draw the rectangle on the screen
    if y2 > y1 and x1 > x2:
        pygame.draw.rect(screen, pygame.Color(color), (x2, y1, widthr, height),
                         width)  # Draw the rectangle on the screen
    if x1 > x2 and y1 > y2:
        pygame.draw.rect(screen, pygame.Color(color), (x2, y2, widthr, height),
                         width)  # Draw the rectangle on the screen
    if x2 > x1 and y1 > y2:
        pygame.draw.rect(screen, pygame.Color(color), (x1, y2, widthr, height),
                         width)  # Draw the rectangle on the screen
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if mode == 'pen':
                pygame.draw.circle(screen, pygame.Color(color), event.pos,
                                   radius)  # Draw a circle (pixel) if the pen mode is active
            prevPos = event.pos  # Store the current position as the previous position

        if event.type == pygame.MOUSEBUTTONUP:
            # When the mouse button is released
            if mode == 'rectangle':
                drawRectangle(screen, prevPos, event.pos, radius,
                              color)  # Draw a rectangle if the mode is set to draw rectangles
            elif mode == 'circle':
                drawCircle(screen, prevPos, event.pos, radius,
                           color)  # Draw a circle if the mode is set to draw circles
            elif mode == 'square':
                drawSquare(screen, prevPos, event.pos, color)  # Draw a square if the mode is set to draw squares
            elif mode == 'right_tri':
                drawRightTriangle(screen, prevPos, event.pos,
                                  color)  # Draw a right triangle if the mode is set to draw right triangles
            elif mode == 'eq_tri':
                drawEquilateralTriangle(screen, prevPos, event.pos, radius,
                                        color)  # Draw an equilateral triangle if the mode is set to draw equilateral triangles
            elif mode == 'rhombus':
                drawRhombus(screen, prevPos, event.pos, radius,
                            color)  # Draw a rhombus if the mode is set to draw rhombuses
            draw = False  # Disable drawing

        if event.type == pygame.MOUSEMOTION:
            # When the mouse is moved
            if draw and mode == 'pen':
                drawLine(screen, lastPos, event.pos, radius,
                         color)  # If drawing is enabled and pen mode is active, draw a line between the last position and the current position
            elif draw and mode == 'erase':
                drawLine(screen, lastPos, event.pos, radius,
                         'white')  # If drawing is enabled and erase mode is active, draw a white line (erase) between the last position and the current position
    ...
    pygame.draw.rect(screen, pygame.Color('white'),
                     (5, 5, 115, 75))  # Draw a white rectangle to display the radius information
